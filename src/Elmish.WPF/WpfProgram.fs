...
module WpfProgram =
    ...
    let mkProgramT
        ...
        (createVm: ViewModelArgs<'model, 'msg> -> 'viewModel)
        =
        Program.mkProgram init update (fun _ _ -> ()) |> createWithVm createVm
    ...
    /// <returns></returns>
    let startElmishLoop (element: FrameworkElement) (program: WpfProgram<'model, 'msg, 'viewModel>) =
        ...
        let measure callName f =
            BindingVmHelpers.Helpers2.measure
                performanceLogger
                LogLevel.Debug
                program.PerformanceLogThreshold
                ""
                "main"
                callName
                f
        ...
        let program =
            { program with
                UpdateViewModel = measure "updateViewModel" program.UpdateViewModel }
     ...
     *)
        ...
        let setUiState model _syncDispatch =
            ...
            | Some vm -> // view model exists, so update
                ...
                | Threaded_UIDispatch uiWaiter -> // We are in the specific dispatch call from the UI thread (see `synchronizedUiDispatch` in `dispatchFromViewModel`)
                    ...
                    element.Dispatcher.InvokeAsync(unscheduleJob, scheduleJobThreadPriority)
                    |> ignore // Unschedule update (already done)
                ...
                | Threaded_NoUIDispatch -> // We are in a non-UI dispatch with no pending user interactions known
                    ...
                    let executeJob () =
                        ...
                        | ValueNone -> updateLogger.LogDebug("Job was empty - No update done {i}", i)
       ...
       *)
        ...
        |> if updateLogger.IsEnabled LogLevel.Trace then
        ...
        fun model ->
            ...
            | ValueNone ->
                let sub =
                    ...
                    |> List.indexed
                    |> List.map (fun (i, subscribe) -> [ idPrefix; string i ], subscribe)
