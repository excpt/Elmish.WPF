...
module internal Helpers =
    ...
    let createBindingT data name = { Name = name; Data = data }
...
module internal ViewModelHelper =
    ...
    let getEventsToRaise newModel helper =
        ...
        |> Seq.collect (fun (Kvp(name, binding)) ->
            Update(helper.LoggingArgs, name).Recursive(helper.Model, newModel, binding))
        |> Seq.toList
...
type internal DynamicViewModel<'model, 'msg>(args: ViewModelArgs<'model, 'msg>, bindings: Binding<'model, 'msg> list) as this
    ...
    let (bindings, validationErrors) =
        let initializeBinding initializedBindings binding =
            Initialize(
                loggingArgs,
                binding.Name,
                ViewModelHelper.getFunctionsForSubModelSelectedItem loggingArgs initializedBindings
            )
        ...
        for b in sortedBindings do
            if bindingDict.ContainsKey b.Name then
                log.LogError(
                    "Binding name {BindingName} is duplicated. Only the first occurrence will be used.",
                    b.Name
                )
            else
                option {
    ...
    interface IViewModel<'model, 'msg> with
        ...
        member _.UpdateModel(newModel: 'model) : unit =
            ...
            let eventsToRaise =
                if prevHasErrors = (helper :> INotifyDataErrorInfo).HasErrors then
                    eventsToRaise
                else
                    (PropertyChanged "HasErrors") :: eventsToRaise
...
type ViewModelBase<'model, 'msg>(args: ViewModelArgs<'model, 'msg>) as this =
    ...
    member _.Get<'a>([<CallerMemberName>] ?memberName: string) =
        fun (binding: string -> Binding<'model, 'msg, 'a>) ->
                ...
                option {
                    ...
                    let! vmBinding =
                        option {
                            ...
                            | true, value -> return value |> MapOutputType.unboxVm
                            | false, _ ->
            ...
            | None ->
                log.LogError(
                    "[{BindingNameChain}] Get FAILED: Binding {BindingName} could not be constructed",
                    nameChain,
                    memberName
                )
            ...
            | Some(Error e) ->
                ...
                | GetError.OneWayToSource ->
                    log.LogError(
                        "[{BindingNameChain}] Get FAILED: Binding {BindingName} is read-only",
                        nameChain,
                        memberName
                    )
                | GetError.SubModelSelectedItem d ->
                    log.LogError(
                        "[{BindingNameChain}] Get FAILED: Failed to find an element of the SubModelSeq binding {SubModelSeqBindingName} with ID {ID} in the getter for the binding {BindingName}",
                        d.NameChain,
                        d.SubModelSeqBindingName,
                        d.Id,
                        memberName
                ...
                | GetError.ToNullError(ValueOption.ToNullError.ValueCannotBeNull nonNullTypeName) ->
                    log.LogError(
                        "[{BindingNameChain}] Get FAILED: Binding {BindingName} is null, but type {Type} is non-nullable",
                        nameChain,
                        memberName,
                        nonNullTypeName
                    )
        ...
        fun (binding: string -> Binding<'model, 'msg, 'a>) ->
            try
                ...
                if success = Some false then
                    log.LogError(
                        "[{BindingNameChain}] Set FAILED: Binding {BindingName} is read-only",
                        nameChain,
                        memberName
                    )
                else if success = None then
                    log.LogError(
                        "[{BindingNameChain}] Set FAILED: Binding {BindingName} could not be constructed",
                        nameChain,
                        memberName
                    )
            with e ->
                log.LogError(
                    e,
                    "[{BindingNameChain}] Set FAILED: Exception thrown while processing binding {BindingName}",
                    nameChain,
                    memberName
                )
