...
module Helpers2 =
    ...
    let measure
        ...
        f
        =
        if not <| logPerformance.IsEnabled(logLevel) then
            f
        else
            fun a ->
                ...
                if sw.ElapsedMilliseconds >= int64 performanceLogThresholdMs then
                    logPerformance.Log(
                        logLevel,
                        "[{BindingNameChain}] {CallName} ({Elapsed}ms): {MeasureName}",
                        nameChain,
                        callName,
    ...
    let measure2 (logPerformance: ILogger) (logLevel: LogLevel) performanceLogThresholdMs name nameChain callName f =
        if not <| logPerformance.IsEnabled(logLevel) then
            ...
            fun a -> measure logPerformance logLevel performanceLogThresholdMs name nameChain callName (f a)


type OneWayBinding<'model, 'a> = { OneWayData: OneWayData<'model, 'a> }

type OneWayToSourceBinding<'model, 'a> = { Set: 'a -> 'model -> unit }
...
type SubModelSelectedItemBinding<'model, 'msg, 'bindingModel, 'bindingMsg, 'vm, 'id> =
    ...
    member b.TypedGet(model: 'model) =
        b.Get model
        |> ValueOption.map (fun selectedId -> selectedId, b.SelectedItemBinding.FromId selectedId)
...
module internal MapOutputType =
    let private baseCase
        (fOut: 'a -> 'b)
        (fIn: 'b -> 'a)
        (data: BaseVmBinding<'model, 'msg, 'a>)
        : BaseVmBinding<'model, 'msg, 'b> =
        match data with
            ...
            SubModelSeqUnkeyed
                { SubModelSeqUnkeyedData =
                    { GetModels = b.SubModelSeqUnkeyedData.GetModels
                      ...
                      CreateCollection =
                        b.SubModelSeqUnkeyedData.CreateCollection >> CollectionTarget.mapCollection fOut
    ...
    let rec private recursiveCase<'model, 'msg, 'a, 'b>
        (fOut: 'a -> 'b)
        (fIn: 'b -> 'a)
        (data: VmBinding<'model, 'msg, 'a>)
        : VmBinding<'model, 'msg, 'b> =
        match data with
...
type FirstValidationErrors() =
    ...
    member this.Recursive<'model, 'msg, 't>(binding: VmBinding<'model, 'msg, 't>) : string list ref option =
        match binding with
...
type Initialize<'t>
    ...
    let measure x =
        x
        |> Helpers2.measure logPerformance LogLevel.Trace performanceLogThresholdMs name nameChain

    let measure2 x =
        x
        |> Helpers2.measure2 logPerformance LogLevel.Trace performanceLogThresholdMs name nameChain
    ...
    member _.Base<'model, 'msg>
        ...
        | OneWayToSourceData d ->
            ...
            { Set = fun obj m -> d.Set obj m |> dispatch } |> OneWayToSource |> Some
        | OneWaySeqData d ->
        ...
        | SubModelWinData d ->
            let d =
                d |> BindingData.SubModelWin.measureFunctions measure measure measure measure2
        ...
        | SubModelSeqUnkeyedData d ->
            let d =
                d
                |> BindingData.SubModelSeqUnkeyed.measureFunctions measure measure measure measure measure2
        ...
        | SubModelSeqKeyedData d ->
            let d =
                d
                |> BindingData.SubModelSeqKeyed.measureFunctions
                    measure
                    measure
                    measure
                    measure
    ...
    member this.Recursive<'model, 'msg>
        ...
        option {
            ...
            | AlterMsgStreamData d ->
                ...
                let dispatch': obj -> unit = d.MapDispatch(getCurrentModel, dispatch)
                let! b = this.Recursive(initialModel', dispatch', getCurrentModel', d.BindingData)
                return { Binding = b; Get = d.Get } |> AlterMsgStream
        }
    ...
    member _.Base<'model, 'msg>(newModel: 'model, binding: BaseVmBinding<'model, 'msg, 't>) =
        ...
        | SubModelWin b ->
            ...
            let close () =
                ...
                | false, _ ->
                    log.LogError(
                        "[{BindingNameChain}] Attempted to close window, but did not find window reference",
                        winPropChain
                    )
                | true, w ->
            ...
            let hide () =
                ...
                | false, _ ->
                    log.LogError(
                        "[{BindingNameChain}] Attempted to hide window, but did not find window reference",
                        winPropChain
                    )
                | true, w ->
            ...
            let showHidden () =
                ...
                | false, _ ->
                    log.LogError(
                        "[{BindingNameChain}] Attempted to show existing hidden window, but did not find window reference",
                        winPropChain
                    )
                | true, w ->
        ...
        | SubModelSeqUnkeyed b ->
            ...
            let create m idx =
                ...
                let args =
                    ViewModelArgs.create
                        m
                        (fun msg -> d.ToMsg (b.GetCurrentModel()) (idx, msg) |> b.Dispatch)
                        chain
                        loggingArgs
        ...
        | SubModelSeqKeyed b ->
            ...
            let create m id =
                ...
                let args =
                    ViewModelArgs.create
                        m
                        (fun msg -> d.ToMsg (b.GetCurrentModel()) (id, msg) |> b.Dispatch)
                        chain
                        loggingArgs
            ...
            with :? DuplicateIdException as e ->
                let messageTemplate =
                    "[{BindingNameChain}] In the {SourceOrTarget} sequence of the binding {BindingName}, the elements at indices {Index1} and {Index2} have the same ID {ID}. To avoid this problem, the elements will be merged without using IDs."
